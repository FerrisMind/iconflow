...
fn render_pack(pack: &NormalizedPack) -> Result<String> {
    let mut out = String::new();
    push_line(&mut out, "// @generated by xtask gen. DO NOT EDIT.");
    // Typed pack surface is crate-private (F-007); keep for discriminant API / future use.
    ...
    push_line(&mut out, "");
    push_line(&mut out, "pub(crate) const FONT_ASSETS: &[FontAsset] = &[");
    for asset in &assets {
        if let Some(feature) = &asset.feature {
    ...
    push_line(&mut out, "pub(crate) enum Icon {");
    for icon in &pack.icons {
    ...
    push_line(&mut out, "            }");
    push_line(&mut out, "        };");
    push_line(&mut out, "        Ok(IconRef {");
    push_line(&mut out, "            family: variant.family,");
    push_line(&mut out, "            codepoint,");
    push_line(&mut out, "        })");
    push_line(&mut out, "    }");
    push_line(&mut out, "}");
    push_line(&mut out, "");
